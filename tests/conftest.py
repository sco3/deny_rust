...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    ...
    if speedup >= 1:
        tr.write_line(f'🚀 Rust is {speedup:.2f}x FASTER than Python', bold=True, green=True)
    else:
        tr.write_line(f'⚠️  Python is {1/speedup:.2f}x faster than Rust', bold=True, red=True)
