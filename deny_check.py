...
# Suppress ALL logging output for cleaner performance metrics
# Configure root logger first to catch everything
logging.basicConfig(level=logging.CRITICAL)
logging.getLogger().setLevel(logging.CRITICAL)

# Suppress plugin logging output for cleaner performance metrics
logging.getLogger("plugins.deny_filter.deny").setLevel(logging.CRITICAL)
logging.getLogger("plugins.deny_filter").setLevel(logging.CRITICAL)
logging.getLogger("mcpgateway").setLevel(logging.CRITICAL)
logging.getLogger("mcpgateway.plugins").setLevel(logging.CRITICAL)
...
def run_test_combination_rs(
    ...
    """Run a single test combination using Rust DenyList.

    Args:
        deny_list_name: Name of the deny list being tested
        deny_list: Pre-initialized DenyList instance (Rust)
        sample_name: Name of the sample text
        sample_text: The text to test
        expected_block: Whether this text should be blocked
    ...
    """
    start_time = time.perf_counter()
    was_blocked = deny_list.scan_str(sample_text)
    elapsed_time = time.perf_counter() - start_time
...
def run_test_suite_rs(config: dict[str, Any], count: int = 1) -> dict[str, Any]:
    """Run the test suite using Rust DenyList approach.
    ...
    Returns:
        Dictionary with summary and detailed results
    """
    from deny_rust import DenyList

    deny_word_lists = config["deny_word_lists"]
    sample_texts = config["sample_texts"]

    print("=" * 80)
    print("DENY FILTER PERFORMANCE TEST (Rust DenyList Mode)")
    print("=" * 80)
    print(f"Deny word lists: {len(deny_word_lists)}")
    print(f"Sample texts: {len(sample_texts)}")
    print(f"Total combinations: {len(deny_word_lists) * len(sample_texts)}")
    ...
    # Create DenyList instances once for each deny list
    print("Creating Rust DenyList instances...")
    deny_lists = {}
    for deny_list in deny_word_lists:
        deny_list_name = deny_list["name"]
        deny_words = deny_list["words"]
        deny_lists[deny_list_name] = DenyList(deny_words)
    print(f"Created {len(deny_lists)} Rust DenyList instances")
    print()
    ...
    while warmup_count < warmup_target:
        for deny_list in deny_word_lists:
            deny_list_name = deny_list["name"]
            deny_words = deny_list["words"]
            deny_list_obj = deny_lists[deny_list_name]
